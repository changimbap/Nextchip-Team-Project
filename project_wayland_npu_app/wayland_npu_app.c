        ...
        }else
        {
            ...
            }else
            {
...
#endif
                {
                    ...
                    nc_rgb_interleaved_to_planar_neon((unsigned char *)v4l2_config[i].video_buf.buffers[video_buf.index].start \
...
void send_v2x_data(pp_result_buf *det_buf, pp_result_buf *lane_buf) {
    ...
    // 화면 해상도 1920 기준 4등분 가상 차선 계산
    float screen_width = 1920.0f; 
    float lane_width = screen_width / 4.0f; 
    ...
    for (int cls = 0; cls < det_buf->draw_info.max_class_cnt; cls++) {
        ...
        // 1. 차종 문자열 매칭 (0:승용차, 1:버스, 2:트럭, 3:오토바이)
        int b_class_id = -1; 
        if (strcmp(class_name, "car") == 0) b_class_id = 0;
        else if (strcmp(class_name, "bus") == 0) b_class_id = 1;
        else if (strcmp(class_name, "truck") == 0) b_class_id = 2;
        else if (strcmp(class_name, "motorbike") == 0 || strcmp(class_name, "bicycle") == 0) b_class_id = 3;
        ...
        for (int bidx = 0; bidx < obj_cnt; bidx++) {
            ...
            // 객체 중심 X 좌표 계산
            float center_x = obj.bbox.x + (obj.bbox.w / 2.0f);
            
            // 2. 가상 차선 판별 (1~4차선)
            int lane_num = 1;
            if (center_x < lane_width) lane_num = 1;
            else if (center_x < lane_width * 2) lane_num = 2;
            else if (center_x < lane_width * 3) lane_num = 3;
            else lane_num = 4;

            // 3. 지정차로 위반 판별 로직
            int event_code = 0; // 0: 정상

            if (b_class_id == 3) {
                // 오토바이는 어떤 차선이든 등장하는 순간 무조건 위반
                event_code = 1; 
                ...
                // 승용차는 2, 3차로가 아니면 위반
                event_code = 1; 
            } else if (b_class_id == 2 && lane_num != 4) {
                // 트럭은 4차로가 아니면 위반
                event_code = 1; 
            }

            // 4. DeepSORT 트래킹 ID 연동 (최근 추가한 로그용 ID)
            int v_id = obj.track_id;
            if (v_id < 0) v_id = 0; // 트래킹 전 예외 처리

            // 5. 구조체에 데이터 포장
            V2X_Data send_packet;
            send_packet.lane = htonl(lane_num);
            send_packet.class_id = htonl(b_class_id);
            send_packet.event_code = htonl(event_code);
            send_packet.vehicle_id = htonl(v_id); 
